...
#### Reference: Jiang P, Green SJ, Chlipala GE, Turek FW, Vitaterna MH (submitted) Reproducible changes in the gut microbiome reveal a shift in microbial and host metabolism during spaceflight.
####
...
#### pc.plane: A numeric vector with length of 2 speciflying the PC plane for PERMANOVA. 
####
#### Returns a list containing: 
#### metadata:         A list containing experimental varibles, variable names, etc.
...
####                     ilr1.PCs:    Values of data1 PCs selected by pc.plane
####                     ilr2.Prj:    Projected values of data2 on the same pc.plane
####                     pamanova1/2: aov tables from PERMANOVA on the pc.plane
...
####                                    Angle1/2:      Angle between the direction of changes and y axis for each dataset.
...
starmaps <- function(data1, meta1, fcol1, data2, meta2, fcol2, mc = 1, nperm = 1000, nboot = nperm,
                  pc.plane = c(1, 2), sbst1 = NULL, sbst2 = NULL){
  ...
  for (i in lowLv:2){
    ...
    output[[LVi]][["ilr1.PCs"]] <- ilr1.PCs[, pc.plane]
    output[[LVi]][["ilr2.Prj"]] <- ilr2.Prj[, pc.plane]

    ## PERMANOVA
    if(is.numeric(cond1)){
      ilr1.PCs_ <- ilr1.PCs[, pc.plane]
      cond1_ <- cond1
    }else{
      sel1 <- which(cond1 %in% sbst1)
      ilr1.PCs_ <- ilr1.PCs[sel1, pc.plane]
    ...
    if(is.numeric(cond2)){
      ilr2.Prj_ <- ilr2.Prj[, pc.plane]
      cond2_ <- cond2
      sel2 <- 1:length(cond2)
    }else{
      sel2 <- which(cond2 %in% sbst2)
      ilr2.Prj_ <- ilr2.Prj[sel2, pc.plane]
    ...
    if(mc > 1){
      ...
      fp.F <- parallel::parSapply(cl, 1:nperm, fperm.adonis2, ilr = ilr2, V = svd1$v, sel = sel2, pc.plane = pc.plane, cond = cond2_)
      parallel::stopCluster(cl);rm(cl)
    }else{
      fp.F <- sapply(1:nperm, fperm.adonis2, ilr = ilr2, V = svd1$v, sel = sel2, pc.plane = pc.plane, cond = cond2_)
    }
    ...
    ## comparing direction
    coef1.pc1 <- permanova1$coefficients[[2,1]]
    coef1.pc2 <- permanova1$coefficients[[2,2]]
    alpha1 <- atan2(coef1.pc1, coef1.pc2)

    coef2.pc1 <- permanova2$coefficients[[2,1]]
    coef2.pc2 <- permanova2$coefficients[[2,2]]
    alpha2 <- atan2(coef2.pc1, coef2.pc2)
    
    cos.theta <- cos(alpha2 - alpha1)
    
    # bootstrap
    if(mc > 1){
      cl <- parallel::makeCluster(mc, type="PSOCK")
      bt.cos <- parallel::parSapply(cl, 1:nboot, boot.angle, prj = ilr2.Prj_, cond = cond2_, alpha1 = alpha1)
      parallel::stopCluster(cl);rm(cl)
    }else{
      bt.cos <- sapply(1:nboot, boot.angle, prj = ilr2.Prj_, cond = cond2_, alpha1 = alpha1)
    }
    ...
    direction <- data.frame(Comparison1 = ifelse(is.null(sbst1), paste("Increase in", fcol1), paste(sbst1, collapse = " vs. ")), 
                            Distance1 = sqrt(coef1.pc1^2 + coef1.pc2^2),
                            Angle1 = 180 * alpha1 / pi,
                            Comparison2 = ifelse(is.null(sbst2), paste("Increase in", fcol2), paste(sbst2, collapse = " vs. ")),
                            Distance2 = sqrt(coef2.pc1^2 + coef2.pc2^2),
                            Angle2 = 180 * alpha2 / pi,
                            cos.theta = cos.theta,
                            P = bt.P)
...
####
#### Produces graphs at each taxa level, each containing 5 panels.
...
####                                        cos.theta = 1, exact same direction; = -1 exact opposite direction; 0, perpendicular
####                         P:             Bootstrap-determined P value of cos.theta != 0  
plot.starmaps <- function(starmap, pdf = "starmaps.pdf", col1 = NULL, pch1 = 21, cex1 = 2, col2 = NULL, pch2 = 18, cex2 = 3, alpha = 0.3){
  ...
  for(lv in levels){
    ...
    ilr1.PCs <- as.data.frame(starmap[[lv]]$ilr1.PCs)
    ilr2.Prj <- as.data.frame(starmap[[lv]]$ilr2.Prj)
    ...
    direction[,c(2,5,7,8)] <- signif(direction[,c(2,5,7,8)], 3)
    direction[,c(3,6)] <- round(direction[,c(3,6)], 1)
...
summary.starmaps <- function(starmap){
  ...
  for(lv in levels){
    ...
    grp2 <- as.character(starmap[[lv]]$direction[1,4])
    F2 <- starmap[[lv]]$permanova2[1, 4]
    pval2s <- starmap[[lv]]$permanova2[1, 6]
    pval2f <- starmap[[lv]]$permanova2[1, 7]
    pval2 <- max(pval2s, pval2f)
    cost <- starmap[[lv]]$direction[1, 7]
    pval3 <- starmap[[lv]]$direction[1, 8]
    omniP <- 1 - prod(1 - c(pval1, pval2, pval3))
    call0 <- ifelse(omniP >= 0.05, "n.s.", ifelse(cost > 0, "Alike", "Opposite"))
    
    tbl.lv <- data.frame(lv, overlap, grp1, F1, pval1, grp2, F2, pval2s, pval2f, pval2, cost, pval3, omniP, call0)
...
#### prj:        A matrix of two columns for the projected values of ilr-transformed data2 on the selected PC plane
#### cond:       A vector specifying selected experimental groups for prj, can be either factor/character or integer/numeric.
boot.angle <- function(boot.i, prj, cond, alpha1){
  ...
  boot.coef.pc1 <- boot.permanova$coefficients[[2,1]]
  boot.coef.pc2 <- boot.permanova$coefficients[[2,2]]
  boot.alpha <- atan2(boot.coef.pc1, boot.coef.pc2)

  boot.cos.theta <- cos(boot.alpha - alpha1)
